#!/usr/bin/env python3
"""
LCA TV - Development Version
Fixed routing for local development without subdirectory issues
"""
...
# Configuration for development (no subdirectory issues)
app.config['SECRET_KEY'] = os.environ.get('SECRET_KEY', 'lcatv-secret-key-dev')
app.config['DEBUG'] = True
# Remove problematic production settings for development
# app.config['APPLICATION_ROOT'] = '/lca'  # Commented out for dev
# app.config['PREFERRED_URL_SCHEME'] = 'https'  # Commented out for dev
...
def login():
    ...
    if request.method == 'POST':
        ...
        print(f"Expected password for {username}: {ADMIN_USERS.get(username, 'NOT FOUND')}")
...
def dashboard():
    ...
    print(f"Dashboard accessed by user: {session.get('user', 'Unknown')}")
...
if __name__ == '__main__':
    print("🚀 Démarrage de LCA TV - Version Development")
    print("=" * 60)
    print("🌐 Site Public:")
